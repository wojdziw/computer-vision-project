...
homographies = generateHomographiesFromPoints(points, 0, fbf)
...
while(ret and frame_index < homographies.shape[0]):
    if paused == False:
        ...
        current = cv2.add(np.float32(new) * np.float32(m) * 0.8,  np.float32(frame_warped))
